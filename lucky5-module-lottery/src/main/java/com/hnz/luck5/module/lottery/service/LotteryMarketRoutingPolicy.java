...
/**
 * Splits a real player's bet between the owner's retained risk and the isolated simulated market.
 * A configured eat amount is a per-period, per-play cap shared by all eat-enabled real players.
 */
@Service
public class LotteryMarketRoutingPolicy {
    ...
    public List<Allocation> allocate(MemberDO member, ChimaConfigDO config, List<BetItemDO> items,
        ...
        for (BetItemDO item : items) {
            ...
            if (eatEnabled) {
                BigDecimal cap = cap(config, item.getPlay());
                BigDecimal remaining = cap.subtract(value(used.get(item.getPlay()))).max(ZERO);
                localAmount = amount.min(remaining);
                used.merge(item.getPlay(), localAmount, BigDecimal::add);
            }
            BigDecimal simulatedAmount = money(amount.subtract(localAmount));
            String routeType = localAmount.signum() == 0 ? "SIMULATED_MARKET"
                    : simulatedAmount.signum() == 0 ? "LOCAL_EAT" : "MIXED";
            result.add(new Allocation(item, money(localAmount), simulatedAmount, routeType));
        }
        return result;
    }

    BigDecimal cap(ChimaConfigDO config, String play) {
        if (config == null || play == null) {
            return ZERO;
        }
    ...
    public record Allocation(BetItemDO item, BigDecimal localAmount, BigDecimal simulatedAmount,
                             String routeType) {
    }
}
