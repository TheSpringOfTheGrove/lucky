package com.hnz.luck5.module.infra.service.codegen.inner;
...
import com.hnz.luck5.framework.apilog.core.annotation.ApiAccessLog;
import com.hnz.luck5.framework.apilog.core.enums.OperateTypeEnum;
...
import com.hnz.luck5.framework.common.util.date.LocalDateTimeUtils;
import com.hnz.luck5.framework.common.util.object.BeanUtils;
import com.hnz.luck5.framework.common.util.object.ObjectUtils;
import com.hnz.luck5.framework.common.util.string.StrUtils;
import com.hnz.luck5.framework.excel.core.annotations.DictFormat;
import com.hnz.luck5.framework.excel.core.convert.DictConvert;
import com.hnz.luck5.framework.excel.core.util.ExcelUtils;
...
import com.hnz.luck5.module.infra.enums.codegen.CodegenVOTypeEnum;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 lucky5-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 lucky5-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.hnz.luck5.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/lucky5-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/lucky5-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "lucky5-module-${table.moduleName}/" + // 顶级模块
                "lucky5-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "lucky5-module-${table.moduleName}/" + // 顶级模块
                "lucky5-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "lucky5-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "lucky5-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "lucky5-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "lucky5-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
