...
- 会员房间退码只能操作自己的未开奖真实订单且要求 `openCancel`；老板后台可以直接退自己盘口的未开奖真实订单，不受会员端 `openCancel` 开关影响。所有退码都必须在事务内恢复余额和总下注并同步消息状态；自动托订单在房间和后台均只读，禁止退码。结算按用户+期号幂等，同开奖结果重复调用直接返回 `alreadySettled`，不同结果拒绝；逐注计算派彩、更新订单/会员/消息，并生成中奖结算消息。
