...
- 房间下注、消息、上下分申请和退码入口都必须由公开 Service 方法建立事务，再在 `TenantUtils.execute(...)` 内完成会员凭据解析和写入；不要依赖同类内部自调用上的 `@Transactional`。
- 房间会话的消息时间字段必须返回 `createdAt`（后台列表兼容字段 `time` 可同时保留）；前端排序必须使用稳定时间和固定消息 ID。轮询刷新只允许在用户原本停留于底部时自动跟随，不能把正在查看历史消息的用户强制拉到底部。
- 公开房间允许会员在停盘时进入、读取会话并发送纯聊天，但任何业务操作都必须统一通过 `requireRoomOperation`：下注、下注预览、查余额、上下分、退码以及快捷指令在 `roomOpen=false` 时一律拒绝，网页群还要检查 `pullEnable`。前端隐藏入口只是交互提示，不能代替后端守卫。
