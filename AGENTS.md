...
- 下注统一先检查房间状态；网页群额外检查 `pullEnable`。配置必须为老板模式才能在本地扣款和生成订单，订单固定写入 `LOCAL_ONLY/NOT_REQUIRED`。普通模式的真实盘口写协议尚未验证，必须在扣款前拒绝，不能伪造网盘或盘口成功。
- 文本指令至少保持：`查`/`余额`、`上[分]金额`、`下[分]金额`、`退[码]订单号`，其它内容进入下注解析。带 `externalId` 的房间消息按会员归属用户幂等；成功下注回复包含期号尾号、期内编号、注数、金额、余额与“点击退码”。
