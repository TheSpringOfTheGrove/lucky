...
- 开奖历史记录使用独立 `/lottery/draws` 轻量接口，支持按期号筛选；页面进入时立即刷新并每 5 分钟自动刷新当前筛选结果，离开页面必须停止定时器，禁止为刷新开奖历史重复加载整套 bootstrap。
...
- 自动开奖只接受开奖 API 连续两次、间隔至少 5 秒返回的同一个非 `00000` 五位号码；状态依次为 `DRAW_ABNORMAL|DRAW_PENDING -> DRAWN -> SETTLING -> SETTLED`。非法号码和 `00000` 必须保留异常状态并停止结算、派奖；结算过程中或结算后的号码冲突不得覆盖原结果，必须留待人工复核。共享期号/号码同步使用独立快速调度，只请求期号与最新开奖页；常态约每 2 秒同步，官方开奖时间前 8 秒至开奖后切换到约 250 毫秒热轮询并复用只读登录会话。每次快照同时保存盘口 `serverTime` 与本机观察时间 `sourceObservedAt`，房间接口按二者推导持续前进的权威时钟，前端只使用该时钟校正本机偏差。老板余额刷新和结算使用其它调度线程，首次合法号码到达后立即作为候选结果给玩家展示，`DRAWN` 提交后即可正式发布，禁止让派奖、返水、吃码或慢账号连接阻塞号码显示。
