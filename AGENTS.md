...
- 会员房间顶部固定显示当前老板房间名、玩家昵称、可用积分、完整当前期号、开盘/封盘/开奖确认/开奖异常状态和封盘倒计时，不显示“本期多少单、多少分”的订单统计；开奖记录栏只显示最新一期的“完整期号 + 开奖时间 + 五位号码球”，点击整行展开近 10 期列表，列表也必须显示完整期号、时间和号码，禁止在这些位置截成后三位造成误解。当封盘期号已经超过最新有效开奖时，顶部必须额外显示最近开奖之后第一期尚未开奖的完整期号和“开奖中”，并与最近开奖栏保留明显间距；号码到达后自动消失，一次跨过多期时不能跳过最早缺失期。顶部不得挤压或覆盖聊天与输入区，刮牌按钮的纵向拖动下限必须避开顶部信息栏。倒计时以接口 `serverTime + remainingSeconds` 校准并每秒显示，禁止只靠页面打开后的本地累计计时。
...
- 会员房间退码只能操作自己的未开奖真实订单且要求 `openCancel`；老板后台可以直接退自己盘口的所有未开奖订单（包括自动托），不受会员端 `openCancel` 开关影响。老板模式或尚未外发的盘口订单可在本地原子退码；已经确认上盘的订单必须先把状态改为 `CANCEL_PENDING` 并在外部盘口退码成功后才返还会员积分，超时或结果不确定不得提前返分。自动托不能从房间端退码。结算按用户+期号幂等，同期开奖结果重复调用直接返回 `alreadySettled`，不同结果拒绝；真实盘口订单存在 `PENDING/SUBMITTING/RETRY/MANUAL_REVIEW/CANCEL_*` 时必须暂停该老板该期结算，禁止在外部状态不确定时派奖。
...
- 盘口只读协议位于 `Wa55MarketClient`：使用浏览器 User-Agent 登录 `/Member/DoLogin`，从 `GetMemberPrint`、`GetCurrentPeriodStatus`、`GetDrawNoTable` 读取余额、期号和开奖，禁止向该类加入写请求。真实下单/退码严格隔离在 `Wa55MarketOrderClient`，分别使用 `/Member/Bet` 和 `/Member/CancelMemberBet`；提交前必须再次核对当前老板盘口期号和开盘状态，成功响应必须校验注单号、流水号、注数及实际受理金额。HTTP 超时、不可解析响应、部分成功和服务重启时遗留的 `SUBMITTING` 都视为资金结果不确定，转 `MANUAL_REVIEW`，禁止自动重投或退款。盘口返回字段存在多种大小写/旧版别名；修改任一客户端必须运行对应 Mock 测试。
