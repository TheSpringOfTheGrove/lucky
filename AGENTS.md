...
- 配置运行模式沿用老板模式开关：`bossMode=true` 为老板模式，真实玩家订单全部留在本地并写 `LOCAL_ONLY/NOT_REQUIRED`；`bossMode=false` 为盘口模式。盘口模式入口和只读连接、余额、期号、开奖能力保留，但当前尚无经过验证的真实盘口下单、退单或派奖写适配器，玩家下注必须返回 `MARKET_ORDER_UNAVAILABLE`，不能伪造提交成功。未取得用户明确授权、单独环境开关并完成小额验证前，严禁调用真钱写接口；自动化测试一律使用老板模式。
...
- 吃码目前是本地经营统计：仅统计当前老板名下开启“吃”的真实玩家订单，自动托和已退码订单排除；投额、派奖和盈亏直接按订单快照计算。真实盘口的吃入/上盘资金路由必须等盘口写适配器协议确认后再实现，禁止用本地模拟账户代替真实结果。
...
- 盘口只读协议已迁移到 `Wa55MarketClient`：使用浏览器 User-Agent 登录 `/Member/DoLogin`，从 `GetMemberPrint`、`GetCurrentPeriodStatus`、`GetDrawNoTable` 读取余额、期号和开奖。该客户端只允许读取连接、真实余额、期号和开奖号码，禁止向其中增加下注等写请求；测试下注统一使用老板模式。盘口返回字段存在多种大小写/旧版别名，特别是 `last_seconds`、`system_db_now` 和五个号码字段；修改解析时要保留前导零并运行 `Wa55MarketClientTest`。
