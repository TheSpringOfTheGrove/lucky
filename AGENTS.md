...
- 会员链接必须适配当前前端的 History 路由：长链接是 `/room?tenantId=...&openId=...`，短链接是 `/r/{openId}?tenantId=...`。不要照搬旧项目的 `/#/room`、`/#/r`，否则会落到后台首页。`/room` 与 `/r/` 都属于未登录白名单。
...
- 自动托余额不足时按 `max(autoTopUpAmount, 本次所需金额-当前余额)` 自动上分；上分记录直接写为“已通过”、生成不可变资金流水和标准机器人通过通知，不进入老板的上下分审核列表。自动托下分同样即时通过，不需要老板审核。自动托资金、消息、返水、吃码、真实会员总数/余额/盈亏、上下分统计和历史业务报表全部排除；业务界面仅在“会员管理”和“订单查询”保留其身份与订单，内部执行表、资金流水和房间消息仅用于链路运行与审计。
...
- 共享开奖同步不得修改任何老板的 `lucky5_system_state.room_open`；老板启动开关与全局期号状态是两个维度。所有老板可收到相同 `OPEN/CLOSED/DRAW_*` 期号，只有本人手动启动且网页群 `pullEnable` 允许时才能下注/上下分。开奖快照必须先为所有老板提交事务，再在独立阶段按 `tenant_id + user_id` 触发幂等结算，避免新老板首次补历史时读取未提交的 `DRAWN` 状态。
