...
- 下注解析器 `LotteryBettingService` 保留旧系统定位、字现、倒、合、上奖、含、取值、重号、兄弟号和五位二定规则；普通定位订单的四字符选择固定表示幸运5后四位“千、百、十、个”，五位开奖号码结算时必须匹配后四位，禁止截取前四位造成百位、十位错位；修改时必须运行 `LotteryBettingServiceTest`，其中包含旧项目快捷指令语料回归。
